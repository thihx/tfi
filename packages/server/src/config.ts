...
export const config = {
  ...
  jobFetchMatchesMs: Number(process.env['JOB_FETCH_MATCHES_MS'] || 10 * 60_000),       // 10 min
  jobPredictionsMs: Number(process.env['JOB_PREDICTIONS_MS'] || 15 * 60_000),           // 15 min
  jobExpireWatchlistMs: Number(process.env['JOB_EXPIRE_WATCHLIST_MS'] || 5 * 60_000),   // 5 min
  jobCheckLiveMs: Number(process.env['JOB_CHECK_LIVE_MS'] || 2 * 60_000),               // 2 min
