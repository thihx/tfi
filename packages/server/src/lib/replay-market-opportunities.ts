...
export function classifyReplayMarketAvailability(
  row: Pick<
    ...
    'playable1x2Home' | 'playableAsianHandicapHome' | 'has1x2Home' | 'hasAsianHandicapHome' | 'hasGoalsOu' | 'hasCornersOu'
  >,
): ReplayMarketAvailabilityBucket {
  if (row.playable1x2Home || row.playableAsianHandicapHome) return 'playable_side_market';
  if (row.has1x2Home || row.hasAsianHandicapHome) return 'side_market_unplayable';
  if (row.hasGoalsOu || row.hasCornersOu) return 'totals_only';
  return 'limited_odds';
}
...
export function buildReplayMarketOpportunity(
  ...
  return {
    ...
    playableAsianHandicapHome: asianHandicapHomeOdds != null && asianHandicapLine != null && asianHandicapHomeOdds >= minOdds,
