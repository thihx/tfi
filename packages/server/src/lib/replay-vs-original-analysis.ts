...
export function evaluatedCasesToCsv(cases: EvaluatedReplayCase[]): string {
  ...
  for (const r of cases) {
    ...
    const row = [
      ...
      csvEscape(r.replayQualityAttribution || ''),
      csvEscape(r.providerCoverageStatus || ''),
      csvEscape(r.replayContextStatus || ''),
      csvEscape(r.llmDecisionDiagnostic || ''),
      csvEscape(r.marketResolutionStatus || ''),
