...
export async function reportRoutes(app: FastifyInstance) {
  // Overview KPIs
  app.get<{ Querystring: PeriodQuery }>('/api/reports/overview', async (req) => {
    return repo.getOverviewReport(parseFilter(req.query));
  });

  // League breakdown
  app.get<{ Querystring: PeriodQuery }>('/api/reports/by-league', async (req) => {
    return repo.getLeagueReport(parseFilter(req.query));
  });

  // Market breakdown
  app.get<{ Querystring: PeriodQuery }>('/api/reports/by-market', async (req) => {
    return repo.getMarketReport(parseFilter(req.query));
  });

  // Weekly time series
  app.get<{ Querystring: PeriodQuery }>('/api/reports/weekly', async (req) => {
    return repo.getWeeklyReport(parseFilter(req.query));
  });

  // Monthly time series
  app.get<{ Querystring: PeriodQuery }>('/api/reports/monthly', async (req) => {
    return repo.getMonthlyReport(parseFilter(req.query));
  });

  // Confidence calibration
  app.get<{ Querystring: PeriodQuery }>('/api/reports/confidence', async (req) => {
    return repo.getConfidenceReport(parseFilter(req.query));
  });

  // Odds range analysis
  app.get<{ Querystring: PeriodQuery }>('/api/reports/odds-range', async (req) => {
    return repo.getOddsRangeReport(parseFilter(req.query));
  });

  // Match minute timing
  app.get<{ Querystring: PeriodQuery }>('/api/reports/by-minute', async (req) => {
    return repo.getMinuteReport(parseFilter(req.query));
  });

  // Daily P/L
  app.get<{ Querystring: PeriodQuery }>('/api/reports/daily-pnl', async (req) => {
    return repo.getDailyPnlReport(parseFilter(req.query));
  });

  // Day of week aggregate
  app.get<{ Querystring: PeriodQuery }>('/api/reports/day-of-week', async (req) => {
    return repo.getDayOfWeekReport(parseFilter(req.query));
  });

  // League × Market cross
  app.get<{ Querystring: PeriodQuery }>('/api/reports/league-market', async (req) => {
    return repo.getLeagueMarketReport(parseFilter(req.query));
  });

  // AI Insights
  app.get<{ Querystring: PeriodQuery }>('/api/reports/ai-insights', async (req) => {
