...
async function run() {
  ...
  const bttsNoRes = await pool.query(`
    ...
    FROM recommendations WHERE result IS DISTINCT FROM 'duplicate'
      AND (selection ILIKE '%btts no%' OR selection ILIKE '%both teams to score: no%' 
           OR selection ILIKE '%no (both%' OR selection ILIKE '%no - both%'
           OR (bet_market ILIKE '%btts%' AND selection ILIKE '%no%'))
    ORDER BY timestamp DESC LIMIT 30
  `);
  ...
  const bttsYesRes = await pool.query(`
    ...
    FROM recommendations WHERE result IS DISTINCT FROM 'duplicate'
      AND (selection ILIKE '%btts yes%' OR selection ILIKE '%both teams to score: yes%'
           OR selection ILIKE '%yes (both%' OR selection ILIKE '%yes - both%'
           OR (bet_market ILIKE '%btts%' AND selection ILIKE '%yes%'))
    ORDER BY timestamp DESC LIMIT 30
  `);
  ...
  log('\n=== ALL BTTS RECOMMENDATIONS (full list) ===');
  const allBtts = await pool.query(`
    SELECT selection, bet_market, minute, score, home_team, away_team,
      result, actual_outcome, confidence, odds, pnl, timestamp::date as date
    FROM recommendations WHERE result IS DISTINCT FROM 'duplicate'
      AND (bet_market ILIKE '%btts%' OR selection ILIKE '%both%team%' OR selection ILIKE '%btts%')
  ...
  `);
  for (const r of allBtts.rows) {
    log(`  [${r.date}] ${(r.home_team+' vs '+r.away_team).padEnd(35)} min=${r.minute} score=${r.score} sel="${(r.selection||'').substring(0,50)}" mkt=${r.bet_market} odds=${r.odds} conf=${r.confidence} => ${r.result||'pending'} pnl=${r.pnl}`);
  }
  ...
  fs.writeFileSync('C:/tfi/market-analysis.txt', output, 'utf8');
  console.log('ANALYSIS_WRITTEN_OK');
  console.log('Lines written: ' + out.length);
}

run().catch(e => { console.error('SCRIPT_ERROR:', e.message); process.exit(1); });
